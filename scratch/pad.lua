...
function pad.set(c, width, height, sticky, screen)
    ...
    local function setscratch(c)
        ...
        --It should check if there is a titlebar first
        --awful.titlebar.remove(c) 
       scratchpad.pad[screen].hidden=true
    ...
    -- If the scratchpad is emtpy, store the client,
...
function pad.toggle(screen)
    ...
    if scratchpad.pad and scratchpad.pad[screen] then
        -- and get it out, to play
        local c = scratchpad.pad[screen]

        -- If it's visible on another tag hide it
        if c:isvisible() == false then c.hidden = true end

        -- Move it to the current worskpace
