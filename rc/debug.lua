...
local function client_info()
    ...
    local ismaster
    if awful.client.getmaster(mouse.screen) == c then 
         ismaster= "Master" 
    else ismaster= "Slave"
    end
    v = v .. ismaster .."\n"
    ...
    naughty.notify({ timeout=10, text = string.format('<span font="Terminus 8">%s</span>', v:sub(1,#v-1)),
		     margin = 10, screen = c.screen })
end
...
function dbg(...)
   ...
   if clients ~= num then
      naughty.notify({ timeout=10, text = text, screen = screen.count() })
   end
end
